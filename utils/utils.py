...
def create_vcal(todo):
    ...
    today = datetime.now(timezone("Asia/Bangkok")).replace(hour=0, minute=0, second=0)
...
def return_calendar_object():
    ...
    calendar = [
        i
        for i in calendars
        if i.get_properties([dav.DisplayName()])["{DAV:}displayname"] == calendar_name
    ][0]
